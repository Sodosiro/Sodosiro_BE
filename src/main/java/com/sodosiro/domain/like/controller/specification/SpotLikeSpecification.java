...
public interface SpotLikeSpecification {
    ...
    @Operation(summary = "일반 관광지 좋아요 토글",
    ...
    @Operation(summary = "인기 관광지 좋아요 토글",
               description = "⚠️ 현재 사용 불가 — 인기 관광지(kakao_spot) 기능 준비 중입니다.",
               deprecated = true)
    ResponseEntity<LikeToggleResponse> toggleKakaoSpotLike(Long userId, Long kakaoSpotId);

    @Operation(summary = "내가 좋아요한 관광지 목록",
               description = "커서 기반 페이지네이션. 첫 요청은 cursor 생략, 이후 응답의 nextCursor 값을 사용하세요.")
    ResponseEntity<MyLikedSpotListResponse> getMyLikedSpots(Long userId, Long cursor, int size);
}
